...
@Injectable()
export class ExtractRoutesService {
    
    extractRoutes(project: Project): { nodes: KGNode[]; relations: KGRelation[] } {
        const nodes: KGNode[] = [];
        ...
        project.getSourceFiles().forEach(file => {
            file.getClasses().forEach(cls => {
                const controllerPath =
                    cls.getDecorator("Controller")?.getArguments()?.[0]?.getText()?.replace(/['"]/g, "") || "";
