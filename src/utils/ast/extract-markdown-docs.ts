...
import { IProjectMarkdown } from "src/modules/project/schemas/markdown.schema";

/**
 * Extracts markdown documentation files and matches them to code nodes
 */
export function extractMarkdownDocs(projectPath: string, nodes: KGNode[]): IProjectMarkdown[] {
    const docs: IProjectMarkdown[] = [];
    ...
    mdFiles.forEach(mdFilePath => {
        ...
        if (relatedNodes.length > 0) {
            doc.relatedNodeIds = relatedNodes.map(n => n.eid);
 ...
 */
function matchMarkdownToNodes(mdFilePath: string, mdFileName: string, mdDirPath: string, nodes: KGNode[], projectPath: string): KGNode[] {
